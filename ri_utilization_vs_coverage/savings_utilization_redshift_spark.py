...
db_table_utilization="acc_cost_v2.savings_utilization"
stage_db_table_utilization='acc_cost_v2.stage_savings_utilization'
...
try:
    utilization_response = client_ce.get_savings_plans_utilization(
        TimePeriod={
            'Start': start_date,
            'End': end_date
        },
...
utilization_data=utilization_response['SavingsPlansUtilizationsByTime']
...
pre_query_utilization = "begin; "\
        "drop table if exists acc_cost_v2.stage_savings_utilization; "\
        "create table acc_cost_v2.stage_savings_utilization as "\
        "select * from acc_cost_v2.savings_utilization "\
        "where 1=2; "\
        "end;"


    
post_query_utilization = "begin; delete from acc_cost_v2.savings_utilization using acc_cost_v2.stage_savings_utilization where acc_cost_v2.stage_savings_utilization.date_dim_id =acc_cost_v2.savings_utilization.date_dim_id; insert into acc_cost_v2.savings_utilization select * from acc_cost_v2.stage_savings_utilization; end;"

utilization_datasink4 = glueContext.write_dynamic_frame.from_jdbc_conf(frame = utilization_dyf, catalog_connection = "acc_redshift", connection_options = {"dbtable": stage_db_table_utilization, "database":db_name,"preactions":pre_query_utilization,"postactions":post_query_utilization}, redshift_tmp_dir = args["TempDir"]+"/savings_utilization", transformation_ctx = "utilization_datasink4")
