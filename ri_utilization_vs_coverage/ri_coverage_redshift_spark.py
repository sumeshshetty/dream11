...
stage_db_table_coverage='acc_cost_v2.stage_ri_coverage'
...
try:
    ec2_coverage_response = client_ce.get_reservation_coverage(
        TimePeriod={
            'Start': start_date,
            'End': end_date
        },
...
rds_coverage_response = client_ce.get_reservation_coverage(
    TimePeriod={
        'Start': start_date,
        'End': end_date
    },
...
redshift_coverage_response = client_ce.get_reservation_coverage(
    TimePeriod={
        'Start': start_date,
        'End': end_date
    },
...
elasticache_coverage_response = client_ce.get_reservation_coverage(
    TimePeriod={
        'Start': start_date,
        'End': end_date
    },
...
ec2_coverage_data=ec2_coverage_response['CoveragesByTime']
for item in ec2_coverage_data:
    item['service_name']='AMAZON EC2'

rds_coverage_data=rds_coverage_response['CoveragesByTime']
for item in rds_coverage_data:
    item['service_name']='AMAZON RDS'

    
redshift_coverage_data=redshift_coverage_response['CoveragesByTime']
for item in redshift_coverage_data:
    item['service_name']='AMAZON REDSHIFT'

elasticache_coverage_data=elasticache_coverage_response['CoveragesByTime']
...
coverage_df.columns = coverage_df.columns.str.replace("Total_", "")
...
post_query_coverage = "begin; delete from acc_cost_v2.ri_coverage using acc_cost_v2.stage_ri_coverage where acc_cost_v2.stage_ri_coverage.date_dim_id =acc_cost_v2.ri_coverage.date_dim_id; insert into acc_cost_v2.ri_coverage select * from acc_cost_v2.stage_ri_coverage; end;"

coverage_datasink4 = glueContext.write_dynamic_frame.from_jdbc_conf(frame = coverage_dyf, catalog_connection = "acc_redshift", connection_options = {"dbtable": stage_db_table_coverage, "database":db_name,"preactions":pre_query_coverage,"postactions":post_query_coverage}, redshift_tmp_dir = args["TempDir"]+"/ri_coverage", transformation_ctx = "coverage_datasink4")
