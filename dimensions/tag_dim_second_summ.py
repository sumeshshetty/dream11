...
wr_df_sec=wr.catalog.table(database=args['raw_database'], table=args['raw_table_second'])
...
###### FOR SECOND SUMMARY #####
colnm_sec=wr_df_sec[wr_df_sec['Column Name'].str.contains("resource_tags_user_")]
...
if  args['delta']=='true':
    ...
    if args['full_load']=='false':
        ...
        print("read one")
        athena_df_sec=wr.athena.read_sql_query(''' SELECT {}
        FROM {}
        GROUP BY  {}
        '''.format(colnm_str_sec,args['raw_table_second'],colnm_str_sec), database=args['raw_database'],chunksize=True)
        ...
        print("len df1:",athena_df_concat.count())
        athena_df_second_list=[]
        for df in athena_df_sec:
            df=create_sha(df)
            athena_df_second_list.append(df)
        
        athena_df_second_concat=pd.concat(athena_df_second_list)
        print("len df2:",athena_df_second_concat.count())
        
        df=athena_df_concat.append(athena_df_second_concat)
        
        
        #df.drop_duplicates(keep='first',inplace=True)
        print("len df3:",df.count())
        print("new formed")
